...
@StepScope
public class TransactionDiaryLedgerProcessor implements ItemProcessor<TradeEntity, TransactionDiary> {
    ...
    private final ParticipantPositionProvider participantPositionProvider;
    ...
    public TransactionDiary process(final TradeEntity trade) {
        ...
        final Optional<DailySettlementPriceEntity> dailySettlementPriceEntity = dspProvider.getDsp(trade.getCurrencyPair().getCode());

        final Optional<ParticipantPositionEntity> sodParticipantPosition =
            participantPositionProvider.getParticipantPositionByCurrencyPairAndParticipantCodeAndType(
                trade.getCurrencyPair().getCode(), originatorParticipant.getCode(), ParticipantPositionType.SOD);
        final String dailyMtMAmount = sodParticipantPosition
            .map(pp -> eodCalculator.calculateDailyMtmValue(pp, dspResolver, jpyRatesResolver).getAmount().toString())
            .orElseGet(() -> eodCalculator.calculateInitialMtmValue(trade, dspResolver, jpyRatesResolver).getAmount().toString());
        @Nullable
        final String swapPoint = sodParticipantPosition
            .map(pp -> eodCalculator.calculateSwapPoint(trade, swapPointResolver, jpyRatesResolver).getAmount().toString())
            .orElse(null);
        @Nullable
        final String outStandingPositionAmount = participantPositionProvider.getParticipantPositionByCurrencyPairAndParticipantCodeAndType(
            trade.getCurrencyPair().getCode(), originatorParticipant.getCode(), ParticipantPositionType.NET)
            .map(pp -> pp.getAmount().toString())
            .orElse(null);
        final String settlementDate = sodParticipantPosition
            .map(pp -> formatDate(pp.getValueDate()))
            .orElseGet(() -> formatDate(trade.getValueDate()));
        ...
        return TransactionDiary.builder()
            ...
            .swapPoint(swapPoint)
            //todo????
            .outstandingPositionAmount(outStandingPositionAmount)
